...
let cosine_sum ?(fftbins = false) (dtype : (float, 'b) Nx.dtype)
    (a : float array) m =
...
let hanning ?(fftbins = false) (dtype : (float, 'b) Nx.dtype) m =
  cosine_sum ~fftbins dtype [|0.5; 1. -. 0.5|] m

let hamming ?(fftbins = false) (dtype : (float, 'b) Nx.dtype) m =
  cosine_sum ~fftbins dtype [|0.54; 1. -. 0.54|] m

let blackman ?(fftbins = false) (dtype : (float, 'b) Nx.dtype) m =
...
let get (typ : window) (dtype : (float, 'b) Nx.dtype) :
    ?fftbins:bool -> int -> (float, 'b) Nx.t =
 fun ?fftbins size ->
  match typ with
  | `Hanning ->
