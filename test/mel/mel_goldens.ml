(* Golden parity against librosa 0.11: filterbank weights (librosa.filters.mel),
   ...
   (librosa.feature.mfcc), replayed from the committed vectors under
   test/vectors/mel/. The test signal is the stft suite's 31-bit LCG under this
...
let vectors_dir = Filename.concat ".." (Filename.concat "vectors" "mel")
