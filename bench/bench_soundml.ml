...
(* {1 The io rows}
   ...
   in bench/bench_sndfile.c and the Python cross-reference in
   bench/bench_soundml_io.py read identical bytes). Row classes, per the io
   performance gates: whole-file reads per format family at both size classes;
   the [Reader.read ?out] form (open, one full-file chunk into a lent
   destination, close — the C ceiling's own shape); the header probe; the
   many-small-files ingest loop; chunked writes; and the fused resampled read
   next to its honest decomposition (native read plus [Resample.apply], config
   built per call on both sides, the way a caller would write it). *)
