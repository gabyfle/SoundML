(* The error matrix: every [error] constructor reachable from an actual
   ...
   malformed-file corpus (corpus/malformed/, MANIFEST-pinned): each malformed
   file, through [info] and [read] at both dtypes, must produce a typed error or
   valid data — never a crash, never zero-fill. Assertions match error variants,
   never libsndfile message strings (the messages travel in [details] and vary
   across libsndfile builds). *)
